...
const Documentpage = () => {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      const data = await getalldata(sort, searchValue,"/text/get-all-blog"); 
      if (data) {
  ...
  const handleDelete = async (id: number | string) => {
     console.log(id)
    try{
        await deleteSingleitem(id,"/text/delete-blog")
    
         toast.success("successfulley delte blog")
         fetchData()
    }catch(err){
    ...
    <div>
      ...
      <li className="shadow-xl lg:px-6 md:px-2 py-8 px-8 rounded-md">
          <span className="flex justify-between gap-20 lg:items-center items-start">
            ...
            <span className="flex flex-col gap-2">
              <h4 className="lg:text-xl md:text-md font-medium ">Documents</h4>
     ...
     <ClipLoader
      ...
      <div className="shadow-xl w-full rounded-lg bg-[#f8faff] p-6">
        <h2 className="text-xl font-semibold dark:text-customDark">All  Docuemts</h2>
        ...
        <div className="overflow-x-auto mt-10">
          <ul className="bg-white shadow-md rounded-lg overflow-hidden">
            ...
            {result?.map((item:TDocument) => (
             <li key={item._id} className="grid space-y-2 md:grid-cols-[1fr_3fr_2fr_1fr_1fr] items-center p-3 text-center shadow-xl hover:bg-gray-50 text-gray-700">
             <Image src={item.coverImage} className=" object-cover md:w-[50px] md:h-[50px]" alt="img" width={400} height={400} />
             ...
             <div className="flex items-center justify-center">
               <span className="bg-blue-400 text-white text-center py-1 rounded-md px-4 md:w-auto w-full">
                 <Link href={`/drashboard/document/updateBlog/${item._id}`}>Update</Link>
               </span>
             </div>
